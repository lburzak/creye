...
class GraphAnalysisService(
    ...
    fun analyze(branch: String): Deferred<DependencyGraph> = scope.async {
        ...
        // Graph construction lands with the dependency-model milestone.
        DependencyGraph()
    }
    ...
    private suspend fun detectChangesNow(branch: String): ChangeDetection {
        ...
        val symbols = readAction {
            val rootPath = GitRepositoryManager.getInstance(project).repositories
                .firstOrNull()?.root?.path ?: project.basePath.orEmpty()

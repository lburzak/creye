...
- Treat identity as run-local: it MUST NOT persist or be compared across runs. Interning a path to a run-local handle is permitted only if it preserves path value-equality and does not derive from discovery order.
...
- Run-local scope follows from the rejected caching ADR-014: nothing compares ids across runs, so no content hashes or persistence-grade ids are needed. Value-equality on the path makes determinism automatic; interning is left available for ADR-007's edge-heavy storage, constrained to stay order-independent so it cannot reintroduce traversal-order dependence and break constraint 1.

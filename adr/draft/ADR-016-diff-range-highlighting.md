...
## Notes
- Open for this draft: the concrete highlight vocabulary and triggers — candidates include highlighting the clicked symbol's body in its file block, tinting hunks that belong to a selected child node, and a transient "focus" highlight on navigation. Deferred until the diff inspection interaction is settled, shared with ADR-015's open question.

...
## Decision
- The comparison branch MUST be selected through a single-select dropdown in the graph window, populated from the repository's current branch list.
...
- The last selection MUST persist in per-project plugin state and MUST be restored when the graph window opens, so the active comparison branch is visible before analysis runs.
...
- The dropdown lives in the graph window header (a Jewel combo box in the ADR-009 Compose tree), so the selected branch is the on-screen state itself, directly satisfying visibility before analysis.
- The core use case feeds the comparison branch into graph generation only; no other feature consumes it, so a dropdown as source of truth avoids a broader project-setting layer that nothing else would read (constraint 5).

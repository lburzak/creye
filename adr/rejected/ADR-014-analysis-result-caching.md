...
- The MVP analyzes changed code only: ADR-003 scopes comparison to the changed-file set including uncommitted working-directory changes, ADR-004 maps hunks to changed declarations, and ADR-006 resolves only those changed declarations' out-edges. The resulting domain graph is small.
...
## Constraints
1. The MVP MUST keep the analysis pipeline responsive and run long-running work off the UI thread (ADR-012 constraints 1–2).
2. The MVP MUST keep plugin packaging thin enough for a single Gradle module with package-enforced boundaries (ADR-001).
...
4. A caching or querying mechanism MUST justify its dependency and complexity against the actual MVP workload, not a hypothetical whole-project one.
...
- **Headroom.** Columnar storage, vector, and full-text indices leave room to grow toward a whole-project or incremental graph beyond the changed-code MVP.

## Rejection reason
The proposed embedded graph database serves no need the MVP actually has:
- **Queries buy nothing.** The changed-code-only graph (ADR-003/004/006) is small; in-memory traversal for ADR-008 aggregation and the ADR-007 projection is simpler and faster than serializing to a store and querying via Cypher. Columnar/Cypher features target large analytical graphs the MVP does not have (constraint 4).
- **Cross-run caching has low payoff.** ADR-003 includes uncommitted changes, so input mutates on every edit and a whole-run cache invalidates constantly. Per-symbol caching is marginal too: by ADR-006 only changed symbols are resolved — an unchanged symbol is never analyzed (nothing to hit) and a changed symbol's hash differs (miss by definition); reuse occurs only on revert/undo/branch toggle. That granularity pays off only for a whole-project / incremental graph, which is out of MVP scope.

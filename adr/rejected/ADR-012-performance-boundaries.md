...
This is rejected as premature rather than deferred-in-draft: there is no decision to make until real graphs prove large. Revisit as a fresh bounds/budget ADR only if validation (ADR-013) shows changed-code graphs growing large enough that ADR-007 neighborhood scoping is insufficient on its own.

...
## Problem & Context
- The analysis focuses on the changed code; the insight is what symbols the changed code depends on, classified by where the target lives, using the internal / external / cohesion classes defined in this record's Decision section.
...
- Full semantic parity with IntelliJ dependency analyzer behavior is not required, and reverse usage search is excluded.
...
2. The implementation MUST classify each dependency edge of the changed code as internal, external, or cohesion per the definitions in this record's Decision section.
...
- Treat the following reference kinds as dependency edges: function and constructor calls, property and field access, type references (parameter, return, supertype, generic argument). Defer annotation references and implicit-receiver / implicit-invoke edges; record each deferred kind as a diagnostic.
...
## Rationale
- Only the dependencies that changed code declares are needed, so reverse usage search (`ReferencesSearch`) is dropped. This removes the expensive, index-fragile half of resolution and confines the whole analysis to local forward inspection of changed code.
...
- K2 is the default frontend and K1 resolution is deprecated, so building on the Analysis API avoids committing to a sunset path.
...
- The chosen edge-kind set covers the dependencies that dominate a representative Kotlin project; deferring annotations and implicit receivers limits scope while diagnostics keep the omission visible.

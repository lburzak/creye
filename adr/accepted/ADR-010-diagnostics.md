...
- Diagnostics MUST be presented through a dedicated IntelliJ tool window, a surface distinct from the graph editor panel (the ADR-009 render surface), styled as a tabs-and-filters panel over the cross-cutting set. The panel MUST NOT be implemented as IntelliJ-native `ProblemsView` integration.
- A diagnostic row that carries an `attachment` MUST navigate to the corresponding graph element in the editor panel; the tool window drives selection in the editor panel rather than hosting the graph itself.
...
- Hosting the panel in a dedicated tool window keeps it a first-class IDE surface alongside the graph editor panel, and lets diagnostic rows drive selection into the graph nodes rendered in the editor panel — graph-node navigation stays native. IntelliJ `ProblemsView` navigates to PSI/`file:line` and cannot target custom graph nodes, which are the primary navigation target.
...
A later additive mirror of file-located diagnostics into IntelliJ `ProblemsView` is not precluded, but the authoritative panel remains the dedicated tool-window view.
